...
class Lstr(object):
    ...
    def isFlatValue(self):
        ...
        """
        return True if ('v' in self._args and not self._args.get('s',[])) else False
...
def _canDisplayLanguage(lang):
    ...
    if lang in ('Chinese','Persian','Korean','Arabic') and bs.getEnvironment()['platform'] in ('windows','linux'):
        return False
    else:
        return True
...
def _getDefaultLanguage():
    langs = { 'de':'German', 'es':'Spanish', 'it':'Italian', 'nl':'Dutch',
              ...
              'ko':'Korean', 'fa':'Persian', 'ar':'Arabic', 'zh':'Chinese', 'tr':'Turkish','id':'Indonesian',
              'uk':'Ukrainian'}
...
def _haveProOptions():
    ...
    #     print 'FORCING FALSE PRO OPTIONS',bs.getConfig().get('lc14292',0)
    #     return False
    # we expose pro options if the server tells us to (which is generally just when we own pro),
    # if we've been grandfathered in,
    # or also if we're a test build..
    return True if (bsInternal._getAccountMiscReadVal2('proOptionsUnlocked', False)
                    or bs.getEnvironment()['testBuild']
